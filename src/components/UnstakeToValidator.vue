      ...
      <v-col>
        <v-row justify="space-between" align="center" class="warn-tip">
          ...
          <v-card outlined class="select-card">
            <v-select class="d-select" :items="itemsPages" label="" outlined item-color="vote_button" dense height="28" v-model="itemsPerPage"></v-select>
          </v-card>
        </v-row>
        <v-data-table :headers="headers" :items="nodeList" class="elevation-0 data-margin" hide-default-footer :items-per-page="itemsPerPage" :no-data-text="$t('msg.nodatatext')" :loading="unStakeListLoading" :loading-text="$t('msg.loading')" :page.sync="page" @page-count="pageCount = $event">
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn small color="vote_button" :disabled="claimStatus(item)" @click.stop="unstake(item)" height="32" style="color: #fff">
              {{ $t('unstake.gettitle') }}
            </v-btn>
          </template>
        </v-data-table>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: this.$t('unstake.unstake_available'),
          value: 'timestamp'
        },
